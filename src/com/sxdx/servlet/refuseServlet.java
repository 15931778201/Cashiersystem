...
public class refuseServlet extends HttpServlet{
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			ps=conn.prepareStatement( "update foodOrderInfo set state='refuse' where customerName=?;");
			ps.setString(1, username);
			rs = ps.executeUpdate();
			if(rs>0)
			{
				 PrintWriter out = response.getWriter();
				 out.print("<script>" + "alert('¾Ü¾ø¶©µ¥³É¹¦');"+"</script>");
				 out.close();
			}
			else
			{
				 PrintWriter out = response.getWriter();
				 out.print("<script>" + "alert('¾Ü¾ø¶©µ¥Ê§°Ü');"+"</script>");
				 out.close();
			}
